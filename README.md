...
**Development status:** published source preview, not yet a certified unattended installation. The Docker image built from `db15e36` passed its Linux source test run (206 passed, 2 skipped) and was deployed with existing named volumes. These checks do not establish a fresh authenticated installation, sustained model continuation or live multi-node collaboration.
